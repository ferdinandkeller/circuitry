...
import { dot_size, dot_radius, cross_width, cross_length, pixel_ratio } from './editor/configuration'
...
export function render_static_background() {
    ...
    // the static background canvas is a virtual canvas which size is a multiple of the dot size
    ...
    for (let x = 0; x < width_point_count; x++) {
        for (let y = 0; y < height_point_count; y++) {
            ...
            static_background_ctx.arc(x * dot_size, y * dot_size, dot_radius/2, 0, 2 * Math.PI)
...
export function render_background() {
    ...
    background_ctx.moveTo(cross_pos.x - cross_length, cross_pos.y)
    background_ctx.lineTo(cross_pos.x + cross_length, cross_pos.y)
    background_ctx.stroke()

    background_ctx.beginPath()
    background_ctx.moveTo(cross_pos.x, cross_pos.y - cross_length)
    background_ctx.lineTo(cross_pos.x, cross_pos.y + cross_length)
    background_ctx.stroke()
}