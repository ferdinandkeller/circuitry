...
import { EditionMode } from "@/modes/edition_modes"
import { ConnectMode } from "./connect"
import { DragMode } from "./drag"
import { PanMode } from "./pan"

export class Modes extends Child {
    active_edition_mode: EditionMode = EditionMode.DRAG_AND_CONNECT

    pan: PanMode = new PanMode(this.circuitry_editor)
    drag: DragMode = new DragMode(this.circuitry_editor)
    connect: ConnectMode = new ConnectMode(this.circuitry_editor)
    ...
    mouse_move() {
        this.circuitry_editor.components_renderer.is_hovering()
    ...
    key_down(keyboard_event: KeyboardEvent) {
        ...
        //     if (this.active_edition_mode === EditionMode.PAN) return
        //     if (this.active_edition_mode === EditionMode.DRAG_AND_CONNECT) {
        //         if (this.drag.is_dragging || this.connect.is_connecting) return
        //     }

        //     this.switch_mode(EditionMode.PAN)
        // }
    }
    ...
    //     // if (this.active_edition_mode === EditionMode.DRA) this.drag.exit_drag()
